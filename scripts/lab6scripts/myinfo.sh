...
Your system has been running for $(if [ $($(uptime | sed -n 's/.* up \([0-9]*\) days,  .*$/\1/p') | wc -w) -ne 0 ]; then echo $(uptime | sed -n 's/.* up \([0-9]*\) days,  .*$/\1/p'); else echo "0"; fi) days, $(uptime | sed -n 's/[^:]*:[^:]*:[^:]*\(..:..\).*$/\1/p' | sed 's/^ /0/p') hours.
The current disk on which your home folder is located is $(df | sed -n '2s/^\([^ ]*\).*$/\1/p') and is $(df | sed -n '2s/^.*\(..\)%.*$/\1/p')% full
